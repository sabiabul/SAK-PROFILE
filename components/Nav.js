...
>
  {navData.map((link,index)=>{
    ...
    rounded-[3px]'>
      ...
      <div className='text-[12px] leading-none font-semibold cap'>
      {link.name}
      </div>
      <div className='border-solid border-l-white border-l-8 border-y-transparent border-y-[6px] border-r-0 -right-2'></div>
    </div>
    <div>{link.icon}</div>
    </Link>
    );
